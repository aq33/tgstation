...
///datum/status_effect/trance/proc/hypnotize(datum/source, message, atom/movable/speaker, message_language, raw_message, radio_freq, list/spans, list/message_mods = list()) //Aquila Edit
/datum/status_effect/trance/proc/hypnotize(datum/source, list/hearing_args) //Aquila Edit
	if(!owner.can_hear())
		return
	//if(speaker == owner) //Aquila Edit
	if(hearing_args[HEARING_SPEAKER] == owner) //Aquila Edit
		return
	var/mob/living/carbon/C = owner
	C.cure_trauma_type(/datum/brain_trauma/hypnosis, TRAUMA_RESILIENCE_SURGERY) //clear previous hypnosis
	addtimer(CALLBACK(C, /mob/living/carbon.proc/gain_trauma, /datum/brain_trauma/hypnosis, TRAUMA_RESILIENCE_SURGERY, hearing_args[HEARING_RAW_MESSAGE]), 10) //Aquila Edit
