...
/datum/quirk/junkie/proc/announce_drugs()
	to_chat(quirk_holder, "<span class='boldnotice'>Masz [initial(drug_container_type.name)] [initial(reagent_type.name)] [where_drug]. Lepiej żeby tyle ci wystarczyło...</span>")
...
/datum/quirk/junkie/on_process()
	...
	if(world.time > next_process)
		...
		if(!H.reagents.addiction_list.Find(reagent_instance))
			...
			to_chat(quirk_holder, "<span class='danger'>Myślałeś, że pokonałeś już nałóg, ale potrzebujesz kolejnej dawki [reagent_instance.name]...</span>")
