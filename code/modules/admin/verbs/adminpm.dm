...
/client/proc/cmd_admin_pm_context(mob/M in GLOB.mob_list)
	...
	if(!holder)
		to_chat(src, "<span class='danger'>Error: Admin-PM-Context: Only administrators may use this command.</span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
		return
	if( !ismob(M) || !M.client )
		return
...
/client/proc/cmd_admin_pm_panel()
	...
	set name = "Admin PM"
	if(!holder)
		to_chat(src, "<span class='danger'>Error: Admin-PM-Panel: Only administrators may use this command.</span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
		return
...
/client/proc/cmd_ahelp_reply(whom)
	if(prefs.muted & MUTE_ADMINHELP)
		to_chat(src, "<span class='danger'>Error: Admin-PM: You are unable to use admin PM-s (muted).</span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
		return
	var/client/C
	if(istext(whom))
		if(whom[1] == "@")
	...
	if(!C)
		if(holder)
			to_chat(src, "<span class='danger'>Error: Admin-PM: Client not found.</span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
		return
...
/client/proc/cmd_admin_pm(whom, msg)
	...
	if(irc)
		...
		if(!msg)
			return
		if(holder)
			to_chat(src, "<span class='danger'>Error: Use the admin IRC channel, nerd.</span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
			return
	...
	if(irc)
		to_chat(src, "<span class='notice'>PM to-<b>Admins</b>: <span class='linkify'>[rawmsg]</span></span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
		...
		if(recipient.holder)
			if(holder)	//both are admins
				...
				to_chat(src, "<span class='notice'>PM to-<b>Admins</b>: <span class='linkify'>[msg]</span></span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
			...
			if(holder)	//sender is an admin but recipient is not. Do BIG RED TEXT
				...
				to_chat(recipient, "<font color='red' size='4'><b>-- Administrator private message --</b></font>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
				to_chat(recipient, "<span class='adminsay'>Admin PM from-<b>[key_name(src, recipient, 0)]</b>: <span class='linkify'>[msg]</span></span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
				to_chat(recipient, "<span class='adminsay'><i>Click on the administrator's name to reply.</i></span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
				to_chat(src, "<span class='notice'>Admin PM to-<b>[key_name(recipient, src, 1)]</b>: <span class='linkify'>[msg]</span></span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
				...
				to_chat(src, "<span class='danger'>Error: Admin-PM: Non-admin to non-admin PM communication is forbidden.</span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
				return
	...
	if(irc)
		...
		for(var/client/X in GLOB.admins)
			if(X.key!=key && X.key!=recipient.key)	//check client/X is an admin and isn't the sender or recipient
				to_chat(X, "<span class='notice'><B>PM: [key_name(src, X, 0)]-&gt;[key_name(recipient, X, 0)]:</B> [keywordparsedmsg]</span>", type = MESSAGE_TYPE_ADMINPM, confidential=TRUE)
