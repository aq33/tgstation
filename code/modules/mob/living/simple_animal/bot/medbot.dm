...
/mob/living/simple_animal/bot/medbot/process_scan(mob/living/carbon/human/H)
	...
	if(assess_patient(H))
		...
		if((last_newpatient_speak + 300) < world.time) //Don't spam these messages!
			var/list/messagevoice = list("[H.name], jak się pan czuje?" = 'sound/voice/medbot/jak_sie_pan_czuje.ogg',"[H.name], piłeś?" = 'sound/voice/medbot/piles.ogg',"[H.name], odstaw te wina!" = 'sound/voice/medbot/wina.ogg')
...
/mob/living/simple_animal/bot/medbot/handle_automated_action()
	...
	if(QDELETED(patient))
		if(!shut_up && prob(1))
			var/list/messagevoice = list("Pavulon już jest." = 'sound/voice/medbot/pawulon_jest.ogg',"Tu jeszcze troszeczke pavuloniku." = 'sound/voice/medbot/jeszcze_pawulon.ogg')
			var/message = pick(messagevoice)
			speak(message)
...
/mob/living/simple_animal/bot/medbot/proc/medicate_patient(mob/living/carbon/C)
	...
	if(C.stat == DEAD || (HAS_TRAIT(C, TRAIT_FAKEDEATH)))
		var/list/messagevoice = list("Pavulonik już leci." = 'sound/voice/medbot/pawulon_leci.ogg',"Słuchaj, mam tysiąc za skóre." = 'sound/voice/medbot/nie_mam_czasu.ogg')
		var/message = pick(messagevoice)
		speak(message)
		playsound(src, messagevoice[message], 50)
		oldpatient = patient
	...
	while(tending)
		...
		if(!treatment_method && emagged != 2) //If they don't need any of that they're probably cured!
			...
			var/list/messagevoice = list("Spierdalaj stary małpiszonie." = 'sound/voice/medbot/malpiszon.ogg',"Pani doktor, pavulon się skończył." = 'sound/voice/medbot/pani_doktor.ogg')
			var/message = pick(messagevoice)
			speak(message)
			playsound(src, messagevoice[message], 50)
			bot_reset()
...
/mob/living/simple_animal/bot/medbot/explode()
	...
	if(emagged && prob(25))
		playsound(src, 'sound/voice/medbot/malpiszon.ogg', 50)
