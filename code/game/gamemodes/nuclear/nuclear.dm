...
/datum/game_mode/nuclear/generate_report()
	return "Jeden ze szlaków handlowych Centrali został niedawno zaatakowany przez kosmicznych piratów. Szukali oni jednego statku - przewozi on broń atomową o wysokim rażeniu, jest ona bezużyteczna bez kodu oraz dysku autoryzacyjnego. Kod został prawdopodobnie szybko znaleziony, lecz dysku nie da się podrobić, wasza stacja posiada jeden taki dysk. Istnieje możliwość że spróbują oni zaatakować stację w celu jego zdobycia oraz użycia broni jądrowej na waszej stacji. Upewnijcie się że dysk jest broniony oraz bądźcie przygotowani na abordaż,"
...
/datum/game_mode/nuclear/generate_credit_text()
	...
	for(var/datum/mind/operative in nuke_team.members)
		round_credits += "<center><h2>[operative.name] jako operator Syndykatu</h2>"
	if(len_before_addition == round_credits.len)
		round_credits += list("<center><h2>Operatorzy wylecieli w powietrze!</h2>", "<center><h2>Ich szczątki nie mogły zostać zidentyfikowane!</h2>")
		round_credits += "<br>"
