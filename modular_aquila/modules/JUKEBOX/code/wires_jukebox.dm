...
/datum/wires/jukebox/get_status()
	...
	status += "Napiêcie serwomechanizmu: [voltage]V."
	status += "Zielona dioda [J.selection_blocked || unpowered ? "nie " : ""]œwieci siê."
	status += "Pomarañczowa dioda [length(J.list_source) != length(SSjukeboxes.song_lib_ranch) || unpowered ? "nie " : ""]œwieci siê."
	return status
