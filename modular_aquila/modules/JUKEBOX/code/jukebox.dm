/obj/machinery/jukebox
	...
	icon = 'modular_aquila/modules/JUKEBOX/icons/jukebox.dmi'
	icon_state = "jukebox"
	verb_say = "states"
	density = TRUE
...
/obj/machinery/jukebox/ui_interact(mob/user)
	...
	if(!user.canUseTopic(src, !issilicon(user)))
		return
	if (!anchored)
		to_chat(user,"<span class='warning'>To urz¹dzenie musi wpierw byæ przykrêcone do pod³o¿a!</span>")
		return
	if(!SSjukeboxes.songs.len)
		to_chat(user,"<span class='warning'>B³¹d: nie znaleziono ¿adnych utworów. Skonsultuj siê z Central¹.</span>")
...
/obj/machinery/jukebox/Topic(href, href_list)
	...
	if(stat & (BROKEN|NOPOWER) || !mains)
		return
	if(!anchored)
		to_chat(usr, "<span class='warning'>To urz¹dzenie musi wpierw byæ przykrêcone do pod³o¿a!</span>")
		return
	if(!allowed(usr) && verify)
		to_chat(usr,"<span class='warning'>Nie masz uprawnieñ, aby korzystaæ z tego urz¹dzenia.</span>")
		playsound(src,'sound/machines/deniedbeep.ogg', 50, 1)
		return
	add_fingerprint(usr)
	if(seconds_electrified)
	...
	switch(href_list["action"])
		if("toggle")
			if(!active)
				...
				if (stop_blocked)
					to_chat(usr, "<span class='warning'>Wciskasz przycisk zatrzymania odtwarzania, ale nic siê nie dzieje. Dziwne.</span>")
				else
					stop = 0
		if("select")
			if(active)
				to_chat(usr, "<span class='warning'>Nie mo¿na wybraæ innego utworu gdy trwa odtwarzanie.</span>")
				playsound(src, 'sound/machines/deniedbeep.ogg', 50, 1)
				return
			if(selection_blocked)
				to_chat(usr, "<span class='warning'>Wciskasz przycisk wyboru utworu, ale nic siê nie dzieje. Smutne!</span>")
				return
...
/obj/machinery/jukebox/proc/attempt_playback()
	if (QDELETED(src))
		return
	if(stop > world.time)
		to_chat(usr, "<span class='warning'>Urz¹dzenie wci¹¿ parkuje p³ytê, spróbuj ponownie za [DisplayTimeText(stop-world.time)].</span>")
		playsound(src, 'sound/machines/deniedbeep.ogg', 50, TRUE)
		return
	if(!activate_music())
		to_chat(usr, "<span class='warning'>B³¹d sprzêtowy, spróbuj ponownie.</span>")
		playsound(src, 'sound/machines/deniedbeep.ogg', 50, TRUE)
		return
	updateUsrDialog()